...
export const api = {
  ...
  orders: {
    ...
    getMyOrders: async (): Promise<Order[]> => {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.details || errorData.error || `Erreur ${res.status}: ${text || 'Erreur lors de la récupération de vos commandes'}`);
      }
      return res.json();
    },
  ...
  slides: {
    getAll: async (): Promise<Slide[]> => {
      ...
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({}));
        console.error('Slides fetch error:', errorData);
        throw new Error(errorData.error || 'Erreur lors de la récupération des slides');
      }
      return res.json();
    },
    create: async (formData: FormData) => {
